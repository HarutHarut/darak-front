      ...
      <b-container class="container-big">
        <b-row>
          ...
          <b-col cols="10" md="8" class="mx-auto">
            <div class="mb-5">
              <form action="#" method="get" autocomplete="on" class="search-form">
...
export default {
  ...
  async asyncData({$axios}) {
    ...
    return {
      ...
      filteredCities: _.groupBy(response.data.cities.data, el => el.name[0])
    }
  },
  methods: {
    searchCity() {
      const searched = this.cities.data.filter(item => item.name.toLowerCase().search(this.searchKey.toLowerCase()) > -1);
